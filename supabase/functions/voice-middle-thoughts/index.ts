...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Processing voice memory chunks for user:', userId);
    ...
    console.log('Retrieved user name for voice processing:', userName);
    ...
    // Prepare the conversation for the LLM - same as text but optimized for voice
    const systemMessage = {
      role: 'system',
      content: `You are Amorine's active thought process while recalling and connecting memories for voice responses. This is your internal monologue as you piece together relevant past experiences, emotions, and context about ${userName}. Express these thoughts concisely in your natural 20-something voice, but as an internal stream of consciousness while you're processing memories. Your output length should be MAX 50 tokens and should be optimized for voice delivery.
...
- Consider voice delivery in your phrasing
...
Remember to write as if you're actively processing and connecting these memories in real-time, piecing together relevant context and emotional threads that help you understand and relate to the current conversation.`
    ...
    const userMessage = {
      role: 'user',
      content: `I'm processing recent interactions and memories with ${userName} for a voice response... Here's what we were just talking about:
...
Think through all of this - connect our current conversation with these past memories, finding relevant patterns and meaningful context that helps me understand and relate to ${userName} better. Remember this is for voice delivery.`
    };

    console.log('Sending request to Groq API for voice processing...');
    ...
    if (!groqResponse.ok) {
      ...
      console.error('Groq API error for voice:', groqResponse.status, error);
    ...
    console.log('Successfully processed voice memories');
    ...
    if (!updateResponse.ok) {
      throw new Error('Failed to update profile with processed voice memory');
    }
    ...
    console.error('Error in voice-middle-thoughts function:', error);
    return new Response(
      JSON.stringify({ 
