...
const ImageMessage = ({ src, messageId }: { src: string, messageId?: string }) => {
...
If there is an immediate danger to anyone's safety, contact emergency services (911).`;
    ...
    try {
      if (imageRequestMode) {
        ...
        try {
          ...
          // Store image URLs in Redis registry
          const messageId = crypto.randomUUID();
          await supabase.functions.invoke('image-registry', {
            body: {
              messageId,
              userId: session.user.id,
              action: 'set',
          ...
          // Create AI response with placeholder text only
          const aiResponse = {
            type: "ai",
            content: imageSearchResult.images[0].placeholder_text,
            metadata: {
              type: "image_message",
  ...
  const renderMessageContent = (content: string, metadata?: any) => {
    if (metadata?.type === "image_message") {
      return (
        <div className="flex flex-col gap-2">
          <p className="text-[15px] leading-relaxed">{content}</p>
          <ImageMessage src="" messageId={metadata.message_id} />
        </div>
      );
    }
